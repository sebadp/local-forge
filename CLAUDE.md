...
- **Datetime tools** (`app/skills/tools/datetime_tools.py`): `_TZ_ALIASES` dict maps Argentine province timezones (e.g. `America/Argentina/Misiones`) to valid IANA zones. `convert_timezone()` uses today's date for time-only inputs (prevents year 1900 bug from `strptime` defaults).
...
- **Agent Metrics & Efficacy (Exec Plan 39)** (`app/database/repository.py`, `app/webhook/router.py`, `app/skills/executor.py`, `app/agent/loop.py`, `app/skills/tools/eval_tools.py`, `scripts/baseline.py`, `scripts/dashboard.py`): 8 new repository methods — `get_tool_efficiency` (avg/max tool calls per trace, LLM iterations, tool error rates by tool), `get_token_consumption` (avg/total input+output tokens from `gen_ai.usage.*` span metadata), `get_tool_redundancy` (same tool+args called >1x in same trace), `get_context_quality_metrics` (fill rate, classify_upgrade_rate, memory relevance proxy from `phase_ab`), `get_context_rot_risk` (correlates fill rate with guardrail pass rate in 2 buckets), `get_planner_metrics` (total sessions, replanning rate, avg replans), `get_hitl_rate` (total HITL escalations, approved vs rejected), `get_goal_completion_rate` (avg of `goal_completion` scores). Scores added: `context_fill_rate` (tokens/limit, 0.0–1.0) and `classify_upgrade` (1.0 when base classify returned "none" and was re-run with context) in `router.py`; `hitl_escalation` (1.0 approved / 0.0 rejected) in `executor.py`; `goal_completion` (LLM-as-judge binary `think=False`) as `asyncio.create_task()` in `agent/loop.py` via `_score_goal_completion()`. Tool `get_agent_stats(days, focus)` added to `eval_tools.py` (focus: all/tools/tokens/context/agent). `scripts/baseline.py` extended with Plan 39 sections. `scripts/dashboard.py` extended with 3 new HTML sections: Tool Calls, Token Consumption, Context Quality & Agent Efficacy cards. `ConversationContext.build_timing` field persists `embed_ms` and `searches_ms` set via `span.set_metadata()` in `phase_ab`. See: [`docs/features/37-metricas_benchmarking.md`](docs/features/37-metricas_benchmarking.md)
- **Ontology Data Model (Exec Plan 42)** (`app/ontology/`): grafo de relaciones SQLite-backed entre entidades del sistema (memories, notes, projects, tasks, topics). `EntityRegistry` — `upsert_entity(type, ref_id, name)` usa `INSERT OR IGNORE` idempotente (<1ms, thread-safe). Relaciones en `entity_relations` con `UNIQUE(source_id, relation_type, target_id)` — `add_relation()` es no-op en duplicados. BFS traversal en `graph.py` — `traverse(registry, start_id, depth=1)`, cap `_MAX_NODES_PER_HOP=10` y `_MAX_RESULTS_PER_TYPE=5` por hop. `enrich_context()` en `enricher.py` — best-effort, retorna `EnrichmentResult(extra_text, entities_found)`. Hooks opcionales en `add_memory/save_note/create_project` en `repository.py` via parámetro `_entity_registry=None`. `ONTOLOGY_SCHEMA` en `db.py` (tablas `entities` + `entity_relations` con índices). Tool `search_knowledge_graph` registrado en categoría `"knowledge"`. `EntityRegistry` inicializado en `main.py` → `app.state.entity_registry`, backfill background task al startup. `get_entity_registry(request)` en `dependencies.py`. `ConversationContext.graph_enrichment: str` populado en `build()` si `entity_registry` disponible — actualmente no inyectado en `_build_context()` (integration point pendiente). Backfill CLI: `scripts/backfill_ontology.py --db data/localforge.db`. `ontology_enabled: bool = True` gatea toda la feature. `metrics_percentiles_enabled: bool = True` — si False, `get_latency_percentiles()` y `get_e2e_latency_percentiles()` retornan `[]` sin hacer queries (guarda memoria). Planner downgrade: en `_run_agent_body()`, si `_run_planner_session` falla → retry una vez → si falla de nuevo: notifica al usuario + score `planner_downgrade=1.0` + fallback a reactive loop. Ver: [`docs/features/42-ontology.md`](docs/features/42-ontology.md)
- **Data Provenance & Lineage (Exec Plan 44)** (`app/provenance/`): `AuditLogger` best-effort — registra mutaciones (CREATE/UPDATE/DELETE/MERGE) sobre memorias, notas y proyectos en `entity_audit_log`. `memory_versions` append-only para historial. `Actor` constants: `user`, `llm_flush`, `llm_consolidator`, `tool`, `agent`, `system`, `file_sync`. Module-level accessor en `app/provenance/context.py` (evita threading de parámetros). `add_memory()` y `save_note()` aceptan `source_trace_id` opcional. Tools: `trace_data_origin` + `get_entity_history` en categoría `"provenance"`. Todos los hooks son `try/except` con `pass` (best-effort). Setting `provenance_enabled: bool = True`. `CommandContext.audit_logger` field. `_get_cmd_audit_logger()` helper en `router.py`. Cleanup: `cleanup_old_entries(days=90)`. Ver: [`docs/features/44-data_provenance.md`](docs/features/44-data_provenance.md)
- **Deployment Maturity (Exec Plan 46)** (`app/health/router.py`, `Dockerfile`, `docker-compose.yml`, `.github/workflows/release.yml`): `/health` es liveness-only (proceso vivo, sin dependency checks, siempre 200). `/ready` es readiness probe (DB ping + Ollama `is_available()`, 503 si alguno falla). `Repository.ping()` — `SELECT 1` lightweight. `ReadinessChecks` + `ReadinessResponse` models en `app/models.py`. Docker: `HEALTHCHECK` con `curl -f http://localhost:8000/health`, `curl` instalado en apt-get, `LABEL org.opencontainers.image.source`. Compose profiles: `dev` (todos los servicios incluyendo Ollama/ngrok), `prod` (sin Ollama/ngrok — externos). `docker compose --profile dev up` / `docker compose --profile prod up`. Release workflow: `.github/workflows/release.yml` — push tags `v*` → build + push a `ghcr.io`. CI apunta a branch `main` (no `master`).
- **Token Accuracy (Exec Plan 45)** (`app/context/token_estimator.py`, `app/llm/client.py`): Runtime calibration del proxy `chars/4` usando `prompt_eval_count` de Ollama. `_token_ratios: dict[str, float]` module-level, keyed por modelo. `calibrate(model, char_count, actual_tokens)` usa EMA (α=0.3) — converge en ~10 requests a <5% error. `estimate_tokens(text, model)` usa ratio calibrado (fallback 4.0). `get_calibration_info(model)` para observabilidad. Calibración llamada en `chat_with_tools()` post-response (zero latency impact). `estimate_messages_tokens()`, `estimate_sections()`, `log_context_budget()` aceptan `model` param opcional. Router pasa `settings.ollama_model` al budget tracking.
- **Operational Automation (Exec Plan 47)** (`app/automation/`): Motor de reglas `IF condition THEN action` evaluadas periódicamente por APScheduler. `AUTOMATION_SCHEMA` en `db.py` — tablas `automation_rules` + `automation_log`. `AutomationRule.from_row()` deserializa rows SQLite. `check_condition()` en `conditions.py` — dispatcher por `condition_type`: `query` (SELECT-only SQL, compara con threshold+operator), `metric` (5 métricas: `guardrail_pass_rate`, `embedding_desync`, `db_size_mb`, `unconsolidated_memories`, `project_inactive_days`), `schedule` (match hora/minuto UTC). `in_cooldown(rule)` previene re-triggers. `execute_action()` en `actions.py` — dispatcher por `action_type`: `notify_user`/`notify_admin` (via PlatformClient), `run_task` (`backfill_embeddings`, `consolidate_memories`, `vacuum_db`), `log`. Fail-safe: todas las acciones en try/except. `evaluate_rules()` en `evaluator.py` — loop principal con cooldown+condition+action+log. `seed_builtin_rules()` en `builtin_rules.py` — 5 reglas INSERT OR IGNORE idempotentes. Integración APScheduler en `main.py` gated por `automation_enabled`. Platform routing: admin_phone con prefijo `tg_` → Telegram. Tools: `list_automation_rules`, `toggle_automation_rule`, `get_automation_log` en categoría `"automation"`. Settings: `automation_enabled: bool = False`, `automation_interval_minutes: int = 15`, `automation_admin_phone: str = ""`. Ver: [`docs/features/47-operational_automation.md`](docs/features/47-operational_automation.md)
- **Metrics Hardening (Exec Plan 38)** (`app/context/token_estimator.py`, `app/database/repository.py`, `app/skills/tools/eval_tools.py`, `app/tracing/recorder.py`, `app/eval/dataset.py`, `scripts/dashboard.py`): `estimate_sections(sections: dict[str, str | None]) -> dict[str, int]` — token estimate per named section (chars/4 proxy). `log_context_budget_breakdown(sections)` — emite INFO con `token_breakdown`, `largest_section`, `total`; nunca WARNING/ERROR. `_compute_percentiles(name, sorted_values)` module-level en `repository.py` — calcula p50/p95/p99 en Python (SQLite no tiene PERCENTILE_DISC). `get_latency_percentiles(span_name, days)` en Repository — si `span_name=None` devuelve top-10 spans por frecuencia. `get_search_hit_rate(days)` en Repository — consulta `json_extract(metadata_json, '$.search_mode')` del span `phase_b`. `ConversationContext.search_stats` field (default `{}`) — populado por `_get_memories_with_threshold()` que ahora retorna `(memories, stats)` en lugar de solo `memories`; modes: `semantic` / `fallback_threshold` / `full_fallback`. Tools `get_latency_stats` y `get_search_stats` en `eval_tools.py`, gated por `tracing_enabled`. `TraceContext` acepta `platform: str = "whatsapp"` — forwarded a `recorder.start_trace()`. `TraceRecorder.start_trace()` incluye `session_id=phone_number` y `metadata.platform` en Langfuse — agrupa conversaciones en Sessions. `TraceRecorder.update_trace_tags(trace_id, tags)` — upsert Langfuse tags (categorías de intent + platform tag) después de classify. `TraceRecorder.sync_dataset_to_langfuse(dataset_name, input_text, expected_output, metadata)` — best-effort push a Langfuse Datasets. `maybe_curate_to_dataset()` acepta `trace_recorder=None` — si provisto, sincroniza golden/correction a Langfuse (failures no se sincronizan — demasiado ruidosos). `scripts/dashboard.py` — HTML autocontenido con Chart.js CDN, `asyncio.run()` sin FastAPI, secciones: summary cards / guardrail pass rates / failure trend / latencias / dataset / recent failures con links a Langfuse. `reports/` en `.gitignore`.
